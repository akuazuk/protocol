...
- [x] **W0.1** Зафиксировать JSON schema shadow payload + attention rules в коде/тестах.
- [x] **W0.2** Unit tests: partial→none; poor@80→softened none; critical+harm→critical;
  plan ensemble downgrade; blocked→none.
- [x] **W0.3** Каталог методиста: новые поля помечены shadow.

### Волна 1 - B1 compute + case UI

- [x] **W1.1** GCE runner: shadow Dx/Plan 1-pass flash, resume, write sidecar.
...
- [x] **W2.1** Фильтр очереди по shadow attention.
- [x] **W2.2** Бейдж poor/critical; primary reason не трогать.
