...
Этот документ - **пошаговый план реализации**, а не только концепция.
...
```
.case-workspace-grid
  .case-workspace-clinical
    [1] clinical document (fields with data-clinical-field)
    [2] zones hero strip (3 big scores + safety)
    [3] criteria table «Как оценивать» (13 rows)
    [4] details: Чек-лист №55 (binary)
    [5] details: Сводный deep + оси A/B/C/D
  .case-workspace-decision
    [6] patient history (moved up - already)
    [7] protocol suggest (вход зоны 2)
    [8] LLM 3Q mapped to zones
    [9] findings (tag zone)
    [10] methodist decision (вердикты = зоны)
    [11] CRM / packs
```

### Шаг 9.2 - Zones hero strip

Три крупные карточки:

- Оформление (№127): pct + band + 1 sentence  
- Диагноз: pct + band + ссылки на clinical_diagnosis / mis_diagnos  
- План по КП: pct + band + kp_status + «N протоколов подобрано»

Safety badge если critical.

Клик по карточке → scroll к criteria group / findings filter.

### Шаг 9.3 - Таблица 13 критериев

Колонки:

Параметр · Регламент · КП? · Оценка (0/0.5/1/n/a) · Как оценивать · Что не так · Поля

Источник: `zones.criteria` (+ how_to_evaluate из YAML).  
Строки с `requires_protocol=true` визуально помечены иконкой «КП».  
n/a reason: `нет prior` / `КП не сопоставлен` / `раздел необязателен`.

Это **главная** таблица методики; rubric в secondary details не дублировать полностью.

### Шаг 9.4 - История → критерии 2c

Под полками истории блок:

> Для коррекций плана (критерии 11–12): prior {date} · tier {ru}.  
> Если prior нет - коррекции не оцениваются (n/a).

Кнопки визитов открывают соседний case (как сейчас).

### Шаг 9.5 - Protocol suggest

Заголовок: «Клинические протоколы для оценки плана (зона План по КП)».  
Если items пуст: banner «КП не сопоставлен - критерии плана не штрафуются за несоответствие протоколу».  
Если items есть: показать top-3 + какие block_scores тянут exam/treatment.

### Шаг 9.6 - LLM блок

Переименовать три вопроса:

- Полнота → Оформление (№127)  
- Диагноз → Диагноз  
- Рекомендации → План по КП  

Рядом мелким текстом proxy % склада vs LLM %.

### Шаг 9.7 - Findings

Каждая карточка: badge зоны (1 / 2a / 2b / safety).  
Фильтр-chips над списком.  
Запрет copy «не соответствует КП» для zone1 codes (проверка в normalize/`title_ru`).

### Шаг 9.8 - Решение методиста

Вердикты уже C/D/R - подписать:

- Полнота = зона Оформление  
- Диагноз = зона Диагноз  
- Рекомендации = зона План по КП  

Сохранение pack пишет `zones_snapshot` (pct/band) в decision_json для обучения.

**Приёмка 9:** gold cases 3650612 / 3643304 / pneumonia-like открываются; критерии 11–12 n/a без prior; suggest banner корректный; вердикты совпадают по смыслу с зонами.
...
1. [ ] Подписи: Оформление (№127) / Диагноз / План по КП - финальные  
2. [ ] Primary шкала = 0/0.5/1 по таблице; binary №55 secondary - да  
3. [ ] Пороги band 50 / 85 - ок или дать другие  
4. [ ] Queue: zone1 bad только узко (missing_both / mo_complete=0) - ок  
5. [ ] Expert portal тоже переводим на зоны на Yesterday - да/нет  
6. [ ] Recompute окно: последние N дней (предложение: 14)  

После чеклиста - команда старта фазы данных:

```bash
scripts/ops/git_task_start.sh mo-zone-scores-engine --pc=pc1 \
  --branch=cursor/mo-zone-scores-engine-pc1
```
