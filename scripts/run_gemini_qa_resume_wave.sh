#!/usr/bin/env bash
# Продолжение после pilot: Wave A (parallel) + merge + batch.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
REPORT_DIR="$ROOT/data/ml/reports"
LOG="$REPORT_DIR/gemini_qa_resume_wave.log"
STATUS="$REPORT_DIR/chunk_qa_progress.md"
POLL_SEC="${POLL_SEC:-120}"

mkdir -p "$REPORT_DIR"
exec >> "$LOG" 2>&1
echo "=== Resume Wave A $(date -Iseconds) ==="
...
Log: `data/ml/reports/gemini_qa_resume_wave.log`
"""
...
echo "Pilot done: $PILOT_N fixes. Keys: $GEMINI_KEYS_N"
WAVE_START=$(date +%s)
...
if [ "$GEMINI_KEYS_N" -ge 2 ]; then
  ...
  write_report "wave_a" "$PILOT_N" 0 "..." "Wave A: 1 ключ"
  "$PY" scripts/llm_chunk_qa.py --queue "$QUEUE" --chunks "$CHUNKS" \
    --out "$ROOT/data/ml/chunk_qa_fixes_wave_a.jsonl" --batch-size 8 &
  PID_WAVE=$!
  while kill -0 "$PID_WAVE" 2>/dev/null; do
    sleep "$POLL_SEC"
    ...
    EL=$(( $(date +%s) - WAVE_START ))
    RATE="~$(( WAVE * 60 / (EL + 1) )) fixes/min"
    write_report "wave_a" "$PILOT_N" "$WAVE" "$RATE" "Wave A (1 key)..."
...
[ -f "$ROOT/data/ml/chunk_qa_fixes_wave_a.jsonl" ] && \
