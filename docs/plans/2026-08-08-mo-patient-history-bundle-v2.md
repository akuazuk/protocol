...
| A1 | `patient_key` (+ опц. `doctor_id`) в `fact_mo_case` + индексы; писать при upsert **каждый** визит дня | дальше |
| A2 | `build_patient_history_bundle` из SQL ленты + тесты | дальше |
| A3 | Одно shadow МО `B_patient_history_context` + UI label | дальше |
| A4 | Wire в deep_eval и case detail (бандл один раз, горизонт = весь склад) | дальше |
| A5 | UI: блок «История пациента» в разборе + бейдж в дне | дальше |
| A6 | (Опц.) нарастающий кэш `fact_mo_patient_history_cache` | дальше |
