...
### Дорожная карта для владельца (без путаницы)

Два **разных** трека. Главный - МКБ на визите. История - потом.

| Шаг | Трек | Что делаем | Статус | Что увидите в МО Аналитика |
|--|--|--|--|--|
| **1** | МКБ | Справочник + сверка названия/кода со справочником на **каждом визите** | **сделано** (#49, GCE) | В разборе случая - замечания «не в справочнике» / «слабо совпадает» (черновик) |
| **2** | МКБ | «Диагноза нет в МО» (`B_dx_absent`) + чип статуса у строки | **сделано** (#49, GCE) | В таблице дня у диагноза: `МКБ ✓` / `нет Dx` / `не в МКБ` / `слабо МКБ`; в разборе - KPI «МКБ / диагноз» |
| **3** | МКБ | Merge PR #49 → deploy GCE | **сделано** (`2026-08-08-073045Z-ci-fix-icd-shadow`) | https://protocol.kravira.by |
| **4** | МКБ | Текст диагноза: слоты → «Диагноз: …» → фрагмент у кода в полном МО | **сделано** (#50, GCE) | Меньше ложных «нет Dx», когда диагноз уехал в другой раздел |
| **4b** | МКБ | Кириллица в коде (`Н 52.1` → `H52.1`) в `mo_icd_resolve` / deep | **в работе** (кейс 3644142) | Нет ложного `B_icd_invalid` при нормальном офтальмологическом Dx |
| **5** | МКБ | Калибровка → замечания МКБ из shadow в **основную** оценку (влияют на балл) | дальше | Чип и findings уже «боевые», не черновик |
| **6** | История | Лента всех визитов пациента + одно МО «уже было у врача/специальности» | **отдельный план, после шагов 1-5** | Блок «История пациента» в разборе со старыми чипами МКБ |

**Итого в аналитике после шагов 1-5 (без истории):**

1. У каждого визита дня - чип МКБ.  
2. В разборе - явные замечания: нет диагноза / нет в справочнике.  
3. История пациента **ещё не** появляется - это шаг 6.

**Когда реализуется «весь план»:** шаги 1-2 уже в коде PR; чтобы увидеть в проде - шаг 3 (merge+deploy); шаги 4-5 - следующие коммиты; шаг 6 - отдельная задача после стабилизации МКБ.
