...
## 11. Актуализация после продолжения

Этот раздел заменяет оперативные SHA и статусы разделов 2-7, но не отменяет
оставшийся scope раздела 8 и полного A01-A32.

- Packaging blocker исправлен и merged в #217 / `ce124215`.
- Подготовленная цепочка #207, #208, #211, #213, #212 и #215 последовательно
  синхронизирована, прошла required CI и merged.
- Guard #216 сохранён; документация #214 согласована с ним и merged как
  `e7121a695b41174a42413d2b937175a0c5fff570`.
- Runtime production выпущен из точного green main
  `246e35336f8b73b3f66e31c38b0d58506c1ce099`.
- Live version: `2026-09-06-115045Z-mo-score-availability-ui`;
  health, MO API, search, security headers, image tag и bind проверены.
- В running image оба lab JSON загружаются; synthetic lab evaluation успешен.
- После оборванного SSH corpus-sync корпус восстановлен штатным скриптом:
  478 PDF и 467 summary; `rag_ready=true`.
- #209 остаётся документационным PR с этим audit/tracker/handoff. #210 содержит
  историческую передачу и после публикации #209 должен быть закрыт как superseded,
  а не merged вторым источником текущего состояния.

Следующий технический этап - A28 (защита от устаревших ответов при быстрой смене
среза), затем оставшиеся блоки evidence/status/history/medications из tracker.
Клинический владелец, mapping №55, веса, primary и blind validation не утверждены.
