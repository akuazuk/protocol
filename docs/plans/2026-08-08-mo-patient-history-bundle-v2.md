...
**Приоритет продукта (владелец):** сначала проверки «диагноз есть в МО» и «диагноз есть в справочнике МКБ» - см. `2026-08-08-mo-icd-name-match-v2.md` §0. Этот план (история пациента) - **второй трек**, усиливает объяснение, не заменяет сверку со справочником.

**Жёсткое требование этого трека:** по каждому ID пациента на складе должна накапливаться **вся** история его визитов (весь период существующих данных, без окна 180д). Не один «прошлый визит», не выборка - полная лента.
