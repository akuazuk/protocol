...
1. Справочник актуален + name_only/directory на текущем визите - сделано (PR).
2. Finding `B_dx_absent` + статус чипа `icd_visit_status` в списке/разборе - **в работе / этот коммит**.
3. Fallback: текст Dx из full-doc около кода, если графа пуста - дальше.
4. Выключить shadow → primary после калибровки - дальше.
