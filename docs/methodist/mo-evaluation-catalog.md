...
### Z.2 Итог МО (пять уровней, overlay)

План: `docs/plans/2026-08-20-mo-grade-ladder-v1.md`. Код: `mo_overall_grade.py`.

| id | UI | Смысл |
|--|--|--|
| `critical` | Критично | риск вреда |
| `important` | Важно | важный риск или диагноз плохо |
| `poor` | Слабо | оформление плохо или план не по подобранному КП |
| `fair` | С замечанием | клиника держится, есть дырки |
| `good` | Хорошо | зоны в норме, риска нет |

Не заменяет зоны. Rceth не входит, пока primary выключен.
