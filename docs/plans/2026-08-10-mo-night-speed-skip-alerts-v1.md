...
- [ ] A1. В `night_mis_pipeline.sh` / cron env: `MO_DAILY_WORKERS=2` (default на VM e2-standard-2).
- [ ] A2. Проброс в `score_inbound_day.sh` → `run_mis_protocol_l1_batch.py --workers`.
- [ ] A3. Smoke на одном дне (например вчера): сравнить wall time и `avg_score` / n cases vs baseline.
- [ ] A4. Если CPU throttle / OOM - откат на `1` одной строкой env.

### B. Skip unchanged (без ломки retry)

- [ ] B1. В status / meta писать `inbound_sha256` (CSV или parquet checksum).
- [ ] B2. Retry: если предыдущий status=`success` **или** sha совпал с уже scored secure day → **не** `--force`, при полном совпадении sha со success - exit 0 early.
- [ ] B3. Main: если sha == last success и cases на месте → score resume only (как сейчас todo=0), не чистить kz_l1_*.
- [ ] B4. Явный `--force` / `MO_NIGHT_FORCE=1` для ручного пересчёта.

### C. Алерт fail

- [ ] C1. После `retry` с status≠success: вызвать существующий `telegram_notify.py` (токены из `.env.gcp-staging`, не в git).
- [ ] C2. Текст без PHI: день, mode, detail (`export_failed` / `doctor_join` / `score`), host=gce.
- [ ] C3. Опционально cron 03:15 UTC: `check_gce_night_status.sh` если status-файл stale/failed.
- [ ] C4. Если Telegram не настроен - писать `ALERT_NEEDED` в лог + non-zero только у checker (night success path не ломать ложным rc от notify).
...
План создан 2026-08-10. Реализация не начата. Оценка: ~0.5–1 день кода + smoke.
