...
LAB_SHADOW_CODES = {CODE_ORDERED_DONE, CODE_PRESENT_GAP}
...
def present_panels(bundle: Mapping[str, Any] | None) -> dict[str, dict[str, Any]]:
    ...
    for day in bundle.get("days") or []:
        ...
        for item in day.get("types") or []:
            ...
            for panel in PANELS:
                if panel.pid in out:
                    continue
                if _type_hits(name, panel):
                    out[panel.pid] = {"id": panel.pid, "label": panel.label, "test_date": test_date}
    return out
...
def build_lab_reconcile(
    ...
    if texts_loaded:
        for pid, row in present.items():
            ...
            if panel and not mentioned_in_mo(exam, recs, panel):
...
def apply_lab_to_result(
    ...
    *,
    ...
    result["lab"] = payload
    extra = lab_shadow_findings(recon)
