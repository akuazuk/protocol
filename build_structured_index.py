...
DIAG_PAT = re.compile(
    r"диагност|критери|симптом|синдром|клиническ|обследован|анамнез|осмотр|"
    r"лабораторн|инструментальн|рентген|кт\s|мрт|узи|экг|мкб|классификац|"
    r"дифференциальн|показан|жалоб|статус|оценк",
    re.I,
)
TREAT_PAT = re.compile(
    r"лечени|терапи|препарат|назначен|дозиров|операц|хирург|консерватив|"
    r"медикамент|неотложн|помощь|режим|реабилитац|профилактик|контрол|"
    r"наблюден|отмен|коррекци|альтернатив",
    re.I,
)
...
def main() -> None:
    ...
    for r in rows:
        ...
        if not diagnosis and paras:
            diagnosis = paras[len(paras) // 4][:MAX_SNIP]
        if not treatment and paras:
            treatment = paras[min(len(paras) // 2, len(paras) - 1)][:MAX_SNIP]
