...
Важно (проверено 2026-07-30): **авто-деплой по push на этом сервисе не срабатывает**.
В настройках стоит `autoDeploy: yes` с триггером `commit`, но за всю доступную историю
деплоев нет ни одного с триггером `new_commit` - все `manual`, то есть кто-то нажимал
кнопку в дашборде. Второй сервис того же репозитория (`protocol-rag`) с теми же
настройками не деплоился с 1 июля, пропустив 159 коммитов в `main`. Похоже на потерянную
связку GitHub → Render (webhook или доступ приложения к репозиторию).

Поэтому push сам по себе прод не обновляет. `render_promote_main.sh` и
