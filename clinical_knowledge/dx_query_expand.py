...
_DX_ALIAS_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(r"гипертон\w*|гипертенз\w*", re.IGNORECASE),
        "артериальная гипертензия гипертоническая болезнь гипертензией повышенным кровяным давлением",
    ),
    (
        re.compile(
            r"\bокс\b|остр\w*\s+коронарн\w*\s+синдром\w*|остр\w*\s+форм\w*\s+ишемическ\w*\s+болезн\w*\s+сердца",
            re.IGNORECASE,
        ),
        "острых форм ишемической болезни сердца окс инфаркт миокарда коронарный синдром",
    ),
    (
        re.compile(r"фибрилляц\w*\s+предсерд\w*", re.IGNORECASE),
        "фибрилляция предсердий фибрилляцией трепетанием предсердий",
    ),
]
