...
**B2C анализы:** файлы с префиксом `a_` (например `a_1.pdf`) - лабораторные анализы для тестов B2C, не заключения (КЗ). В batch-метриках КЗ они помечаются `doc_kind=b2c_analysis` и исключаются флагом `--kz-only`.
