...
### Gate S2 end-to-end (закрыт, июль 2026)

Прогон роутера `query_route_v1` на 100 symptom-запросах (`run_symptom_icd_probe`) против существующего no-gemini пайплайна:

| Путь | Top-1 ICD-глава plausible | Латентность |
|------|---------------------------|-------------|
| Существующий no-gemini пайплайн (baseline) | **100%** (0 bad, 0 exotic) | ~150 мс |
| S2 роутер `query_route_v1` | **17%** | ~10 мс |

**Вердикт: gate FAIL, роутер отклонён (`rejected`).** Причины:
1. LOO-CV 88% был in-sample на KZ-выдержках (длинный формальный текст с диагнозом); на коротких симптом-запросах без диагноза модель вырождается в доминирующие классы (L/M).
2. Датасета 84 строки мало и он несбалансирован (нет глав F/G/C/O/S/T в обучении).

**Главный вывод:** отдельный S2 для ICD-плаузибельности **не нужен** - существующий детерминированный лексиконный no-gemini путь уже даёт 100% top-1 при ~150 мс. Ускорять ICD-шаг обучаемой моделью нет смысла; Gemini в этом шаге и так не обязателен. Shadow-хук оставлен только для сбора реальных `query→ICD` меток (D5) на случай будущей задачи, где лексикон недостаточен.

Это пример работы барьера §6.2: gate поймал модель, которая выглядела отлично in-sample (88%), но проваливает реальную задачу (17%), и не пустил её в прод.

### Интеграция и реестр моделей

- **S2 shadow-режим:** модуль `clinical_knowledge/query_router.py` (ленивая загрузка `query_route_v1`, graceful fallback при отсутствии модели). Хук в `_infer_icd_pipeline_from_full_query` логирует предсказание роутера рядом с фактическим путём Gemini/эвристики, **не влияя на ответ**. Включается флагом `RAG_QUERY_ROUTER_SHADOW=1` (по умолчанию off). **Текущий `query_route_v1` отклонён по gate (см. выше)** - в горячий путь не пойдёт; shadow-хук используется только для сбора меток под будущее переобучение.
