...
## 9. Решения владельца (нужны до кода UI)

1. **№55 кольцо на Сегодня:** да на hero (рекомендация по этому запросу) или только в «Подробнее» (строгий zones-first)?
2. **Окно аналитики:** всегда следует `#period` (рекомендация) или отдельный переключатель «14 дней / 7 / месяц» только на графике?
3. **Динамика при `period=month`:** дни или сразу недели?
4. **МКБ-кольцо:** в волне 1 или отложить в волну 2?
5. **Сейчас запускать волну 0 (recompute + API lookback)** на GCE до UI?
...
3. [ ] Ответы на §9.
4. [ ] Волна 0: API + queue + recompute имеющихся дней.
5. [ ] Волна 1: кольца + динамика на Сегодня.
6. [ ] Волна 2: Период parity / МКБ / stacked №55.
7. [ ] BUILD_VERSION / PR / smoke `/methodist/mo` + GCE.

Код UI не начинать до §9; волну 0 API можно стартовать сразу после «да» на п.5.
