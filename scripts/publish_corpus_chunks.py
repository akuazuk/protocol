...
def main() -> int:
    ...
    if replacing:
        # Отказ здесь несущий, а не перестраховка. Манифест агрегирует по
        # source_path и пишет byte_offsets со всех частей, а source_part
        # запоминает только первую (build_corpus_path_manifest.py:63-64, 81).
        # Если путь окажется в двух частях, смещения второй будут читаться из
        # файла первой - то есть по неверным позициям.
        #
        # Защита в LazyChunkStore._rows_match_entry от этого не спасает: она
        # смотрит только первую строку с source_path. Первые смещения - из
        # правильной части, проверка проходит, и мусор с чужих позиций
        # принимается как текст этого протокола. Врач получил бы фрагменты
        # других документов под именем нужного.
        #
        # Правильное переиздание = перезапись части целиком, отдельная задача.
        print(
            "\nпереиздание существующих путей не поддерживается: манифест не умеет\n"
            "описывать один путь в двух частях - смещения будут читаться из чужого\n"
            "файла, и в текст протокола попадут фрагменты других документов.\n"
            "Нужна перезапись части целиком, см. docs/plans/2026-09-05-corpus-integrity-v1.md",
            file=sys.stderr,
        )
        return 3
