...
CREATE TEMP TABLE IF NOT EXISTS fact_mo_lab_stage (
  patient_key TEXT NOT NULL,
  test_date TEXT NOT NULL,
  test_id INTEGER NOT NULL,
  type_id INTEGER,
...
def _ensure_unique_rows(db: sqlite3.Connection) -> None:
    """One-time dedup migration before creating the exact-row unique index."""
    if db.execute(
        "SELECT 1 FROM sqlite_master WHERE type='index' AND name='ux_lab_exact_row'"
    ).fetchone():
        return
    db.execute(
        """
        DELETE FROM fact_mo_lab
        WHERE rowid NOT IN (
          SELECT MIN(rowid)
          FROM fact_mo_lab
          GROUP BY patient_key, test_date, test_id, type_id, type_name,
                   indicator_id, indicator_name, value, unit
        )
        """
    )
    db.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS ux_lab_exact_row
        ON fact_mo_lab(
          patient_key, test_date, test_id, type_id, type_name,
          indicator_id, indicator_name, value, unit
        )
        """
    )
...
def ingest(out: Path, date_from: date, date_to_exclusive: date) -> dict[str, object]:
    ...
    try:
        for start, end in _month_starts(date_from, date_to_exclusive):
            ...
            cur.close()
            print(
                f"chunk {start.isoformat()}..{end.isoformat()} staged",
                flush=True,
            )
    finally:
        mis.close()
