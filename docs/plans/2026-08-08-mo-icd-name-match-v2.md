...
|--|--|--|--|--|
| **1** | МКБ | Справочник + сверка названия/кода со справочником на **каждом визите** | **сделано в PR #49** (пока shadow) | В разборе случая - замечания «не в справочнике» / «слабо совпадает» (черновик) |
| **2** | МКБ | «Диагноза нет в МО» (`B_dx_absent`) + чип статуса у строки | **сделано в PR #49** (чип; findings shadow) | В таблице дня у диагноза: `МКБ ✓` / `нет Dx` / `не в МКБ` / `слабо МКБ`; в разборе - KPI «МКБ / диагноз» |
| **3** | МКБ | Merge PR #49 → deploy GCE | **дальше (нужен merge)** | То же на https://protocol.kravira.by |
| **4** | МКБ | Текст диагноза искать и рядом с кодом в полном МО, если графа пуста | дальше | Меньше ложных «нет Dx», когда диагноз уехал в другой раздел |
