...
3. **Поиск:** демо «I10» / «M32.9 СКВ» или учебный кейс → «Найти протоколы» (быстрее без inline-выдержки; кнопка «Загрузить развёрнутую выдержку»).
4. **Проверка КЗ:** PDF или «Демо-текст КЗ (СКВ)» → сохранить в PDF → «Проанализировать».
5. Буклет: [docs/ministry-brief-print.html](docs/ministry-brief-print.html) → печать в PDF.
