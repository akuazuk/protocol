...
def enrich_business_plan(doc: Document, charts: dict[str, Path]) -> None:
    ...
    appendix_sections: list[tuple[str, str, list[tuple[str, ...]] | None, str | None]] = [
        ...
        (
            ...
            "Каркас ml/, fine-tune e5, A/B consult_gold; отчёты ml/experiments/.",
            [
                ("Компонент", "Механизм", "Обучение"),
                ("send_gate", "правила", "не обучается"),
                ("RAG", "embedder + reranker", "LoRA, exp. MRR +0,29"),
                ("required_exam", "entailment", "метки методиста"),
                ("A/B KZ RAG", "топ-5 КП", "100% / 100%"),
                ("A/B golden", "retrieve 18", "77,8% / 77,8%"),
                ("Фаза 1", "Q4 2026", "деплой local embedder"),
            ],
            None,
        ),
    ]
