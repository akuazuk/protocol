...
Фаза 3.6 (навигатор по протоколу `proto-viewer.html`, сделано):
- Причина «обрывков» на странице «Навигация по протоколу»: она рендерила только `/api/protocol-source-text`
  (сырые пункты из PDF - лиды/буллеты, часто обрывочные).
- Новый endpoint `GET /api/protocol-card` (`rag_server.py`): тот же проектор, что и в поиске
  (Summary Card -> контроль качества -> обогащение страниц по `page_locator`), для одного протокола.
- `proto-viewer.html`: сверху добавлена панель **«Ключевые выдержки»** - целые фразы по разделам с
  вкладками (Всё/Критерии/Обследования/Лечение/Красные флаги/Наблюдение), «стр. N» и «✓ цитата»;
  полный текст по разделам остаётся ниже для глубокого просмотра/поиска. Панель показывается только
  когда карточка доступна (иначе - без изменений).
- Усилен контроль качества (`extract_quality.py`): срез теговых префиксов `[routing]`/`[...]` и
  отсев юридического/оглавительного boilerplate («термины и их определения», «О здравоохранении»,
  «общие положения», «настоящий клинический протокол» и т.п.). Тесты добавлены. `BUILD_VERSION` r16.
- После итерации 2 (LLM-переизвлечение) панель станет качественной на всех ~477 КП.

Фаза 3.7 (авто-исправление раскладки, сделано):
- Запросы, набранные латиницей по ошибке (`ntvgthfnehf b rfitkm` = «температура и кашель»),
  теперь обрабатываются: `clinical_knowledge/keyboard_layout.py::maybe_fix_layout` конвертирует
  QWERTY -> ЙЦУКЕН и включает исправление, только если результат похож на русский клинический
  запрос (совпала медицинская основа слова). Латинские препараты/коды МКБ (`amoxicillin`, `J45`)
  и уже-русские запросы не трогаются.
- Подключено в `rag_server.py` до анализа МКБ и RAG (`_infer_icd_pipeline_from_full_query` и
  `/api/search/protocols-by-icd`). В ответе assist - `query_layout_corrected` и
  `query_before_layout`; в UI (`index.html`) в контексте воронки показывается «Исправлена
  раскладка (было: …)». Тесты: `tests/test_keyboard_layout.py`. `BUILD_VERSION` r17.
