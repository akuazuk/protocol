(function () {
  ...
  function uploadJokeRetryHint(pr) {
    var kind = (pr && pr.guessed_kind) || "";
    if (kind === "lab_in_kz") {
      return "Выберите заключение после приёма (КЗ, медосмотр или консультацию). Бланк анализов можно добавить в блок «Анализы» ниже.";
    }
    if (kind === "kz_in_lab") {
      return "Заключение после приёма - в первый блок, анализы - во второй.";
    }
    return "";
  }
