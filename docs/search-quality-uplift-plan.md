...
Фаза 2 (UI, следующее): переписать `renderProtocolNavCard`/`renderProtoLi` под `protocol_card`
(название сверху, выдержки по разделам, чипы care_setting/МКБ), убрать сырой блок для врача.
Фаза 3: значки опоры/проверки и ссылки на страницу на каждой выдержке.
