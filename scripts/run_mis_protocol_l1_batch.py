...
def build_summary(cases: list[dict], *, month: str, source: str) -> dict:
    ...
    return {
        ...
        "filials": filials,
        "top_doctors": doctors[:15],
        "bottom_doctors": list(reversed(doctors[-15:])) if len(doctors) >= 15 else list(reversed(doctors)),
        "notes": [
...
def main() -> int:
    ...
    if args.rebuild_summary_only:
        ...
        print(f"rebuilt summary n={len(cases)} -> {summary_path}")
        return 0
    ...
    # keep last result per visit_id
    by_vid: dict[str, dict] = {}
    for c in all_cases:
        vid = str(c.get("visit_id") or "")
        if vid:
            by_vid[vid] = c
    summary = build_summary(list(by_vid.values()), month=args.month, source=str(args.csv))
    summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    print(
        f"DONE ok={ok} fail={fail} total_unique={len(by_vid)} "
        f"avg={summary.get('avg_overall_pct')} -> {summary_path}",
        flush=True,
    )
