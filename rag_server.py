...
def api_methodist_mo_case_detail(
    ...
    if run_live:
        try:
            ...
            try:
                pipe = evaluate_mo_icd_match(live_case)
                result["icd_match"] = {
                    "engine": pipe.get("engine"),
                    "diag_source": pipe.get("diag_source"),
                    "codes": pipe.get("codes"),
                    "alias_expanded": pipe.get("alias_expanded"),
                    "mis_agreement": pipe.get("mis_agreement"),
                    "pipeline_verdict": pipe.get("pipeline_verdict"),
                    "score_pct": pipe.get("score_pct"),
                    "needs_llm_review": pipe.get("needs_llm_review"),
                    "chip": pipe.get("chip"),
                }
                chip = pipe.get("chip") if isinstance(pipe.get("chip"), dict) else None
                if chip and chip.get("status"):
                    result["icd_visit_status"] = chip
                else:
...
def api_methodist_mo_protocol_suggest(
    case_id: str,
    request: "Request",
    month: str = Query("", max_length=7),
) -> dict:
...
def api_methodist_mo_drugs_labs_kpis(
    ...
    """KPI unused lab + drug-safety (план 2026-09-04, shadow по умолчанию)."""
    _require_methodist_auth(request)
    from clinical_knowledge.mo_backend import build_mo_drugs_labs_kpis

    return build_mo_drugs_labs_kpis(
        _mo_params(date_from=date_from, date_to=date_to)
    )
