...
def build_scoring_info() -> dict[str, Any]:
    ...
    return {
        ...
        "severity": [
            {"key": "P0", "label": "Критично", "desc": f"Риск вреда пациенту или критический дефект. Ограничивает overall до {p0_cap}, статус записи «критично»."},
            {"key": "P1", "label": "Важно", "desc": f"Клинически важно (безопасность, диагноз, лечение). Ограничивает overall до {p1_cap}, статус «на разбор»/«плохо»."},
            {"key": "P2", "label": "Умеренно", "desc": "Умеренный сигнал (например Moderate DDI), без немедленного риска вреда."},
            {"key": "P3", "label": "Оформление", "desc": "Замечание по полноте или оформлению записи."},
        ],
        "risk_gate": [
            f"Подтверждённое «Критично» (trust A/B) → overall ≤ {p0_cap}, статус записи «критично».",
            f"Подтверждённое «Важно» (trust A/B) → overall ≤ {p1_cap}, статус «на разбор».",
            "«Критично»/«Важно» с trust C/D не штрафуют балл: advisory и очередь методисту.",
